...
The application meant to be deployed is the **Item catalog app**, previously developed for [Project 3](https://github.com/mvuijk/build-an-item-catalog-application).

## Useful info

IP address: 35.176.251.173

Accessible SSH port: 2200

Application URL: [http://35.176.251.173.xip.io/](http://35.176.251.173.xip.io/)

## Step by step walkthrough

### 1 - Create a LightSail AWS EC2 instance.
1. Folow the steps: [Get Started with Lightsail](https://classroom.udacity.com/nanodegrees/nd004/parts/ab002e9a-b26c-43a4-8460-dc4c4b11c379/modules/357367901175462/lessons/3573679011239847/concepts/c4cbd3f2-9adb-45d4-8eaf-b5fc89cc606e)

### 2 - Create a new user named *grader* and grant this user sudo permissions.

1. Log into the remote VM as *ubuntu* user through ssh: `ssh ubuntu@35.176.251.173 -i ~/.ssh/LightsailDefaultPrivateKey-eu-west-2.pem`.
2. Add a new user called *grader*: `2. Add a new user called *grader*: `$ sudo adduser grader`.
...
4. Now you are able to log into the remote VM through ssh with the following command: `ssh grader@35.176.251.173 -i ~/.ssh/udacity_key`
...
2. `$ sudo service ssh restart`.
3. Now you are able to log into the remote VM through ssh with the following command: `$ ssh grader@35.176.251.173 -p 2200 -i ~/.ssh/udacity_key`.
...
3. Move inside that newly created folder: `$ cd /catalog` and clone the catalog repository from Github: `$ git clone https://github.com/mvuijk/build-an-item-catalog-application.git`.
...
```
5. Some tweaks were needed to deploay the catalog app, so I made a [*deployment*](https://github.com/mvuijk/build-an-item-catalog-application/tree/deployment) branch which slightly differs from the *master*. Move inside the repository, `$ cd /var/www/catalog/catalog` and create branch with: `$ git checkout -b deployment`.

**Notes**: the *.git* folder will be inaccessible from the web without any particular setting. The only directory that can be listed in the browser will be the static folder: [static assets](http://35.176.251.173.xip.io/static/).
...
2. If *virtualenv* is not installed, use *pip* to install it using the following command: `$ sudo pip3 install virtualenv`.
...
6. Install Flask: `$ sudo pip3 install Flask`.
7. Install all the other project's dependencies: `$ sudo pip3 install bleach httplib2 request oauth2client sqlalchemy python-psycopg2`. 
...
```
<VirtualHost *:80>
    ServerName 35.176.251.173
    ServerAdmin admin@35.176.251.173
...
### 18 - Create Google Authentication
 
1. Go to [Google Developers Consul](https://console.developers.google.com) and create a new application.
2. Create a new OAuth Client ID (wihtin the Credentials tab)
3. Under Javascript origins, include the following:
   * `http://35.176.251.173.xip.io`
4. Under Authorized redirect URIs:
   * `http:///35.176.251.173.xip.io/login`
   * `http:///35.176.251.173.xip.io/gconnect`
   * `http:///35.176.251.173.xip.io/`
5. Download the JSON file with ID's and replace the contents of the `client_secrets.json` file.
6. Update line 14 with the new client ID in /catalog/templates/login.html file.
7. Update line 6 with `<script src="https://apis.google.com/js/client:platform.js?onload=start" async defer></script>`
   * See [this comment](https://discussions.udacity.com/t/linux-server-configuration-amazon-lightsail-localhost-setup/583077/26) in forum for details
8. Update lines 30 & 217, update the file path for `client_secrets.json`

### 19 - Restart Apache to launch the app
1. `$ sudo service apache2 restart`.

#### Special thanks to [*stueken*](https://github.com/stueken) and and [*ianjanicki*](https://github.com/ianjanicki) who wrote really helpful README's in their repositories.
*steuken* - [repository](https://github.com/stueken/FSND-P5_Linux-Server-Configuration)

*ianjanicki* - [repository](https://github.com/ianjanicki/udc_linux-server-configuration)
